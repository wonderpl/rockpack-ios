				...
				case 2:
					switch(num_leading_hyphens) {
						case 0:
							if (*ch == '-') {
								...
								if (*++ch == 'W')
									...
									// DebugLog(@"(%@) parsing month; segment is %lu and ch is %s", string, (unsigned long)segment, ch);
									switch(num_digits) {
										case 4: //YY-MMDD
											day = segment % 100U;
											month_or_week = segment / 100U;
						...
						case 1:; //-YY; -YY-MM (implicit century)
                                 //DebugLog(@"(%@) found %lu digits and one hyphen, so this is either -YY or -YY-MM; segment (year) is %lu", string, (unsigned long)num_digits, (unsigned long)segment);
							...
							if (*ch == '-') {
								++ch;
								month_or_week = read_segment_2digits(ch, &ch);
								//DebugLog(@"(%@) month is %lu", string, (unsigned long)month_or_week);
							}
