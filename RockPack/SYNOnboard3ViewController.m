//
//  SYNOnboard1ViewController.m
//  rockpack
//
//  Created by Nick Banks on 20/06/2013.
//  Copyright (c) 2013 Nick Banks. All rights reserved.
//

#import "SYNOnboard3ViewController.h"

@interface SYNOnboard3ViewController ()

@end

@implementation SYNOnboard3ViewController

- (void) viewDidLoad
{
    [super viewDidLoad];
    
    // Set up onboard text
    self.onboardMessageLabel.text = NSLocalizedString(@"startscreen_onboard_3", @"Text for onboard screen 3");
    self.onboardTitleLabel.text = NSLocalizedString(@"startscreen_onboard_3_title", @"Text for onboard screen 3");

    
}

@end
