...
- (void) viewDidLoad
{
    ...
    [self.recentFriendsCollectionView registerNib: [UINib nibWithNibName: @"SYNOneToOneSharingFriendCell"
                                                                  bundle: nil]
                   ...
                   cellForItemAtIndexPath: (NSIndexPath *) indexPath
{
    ...
    if (realIndex < self.recentFriends.count) // real recent friends cell
    {
        ...
        if (friend.displayName && ![friend.displayName
                                    isEqualToString: @""])
        {
            nameToDisplay = friend.displayName;
        }
        else if (friend.email && ![friend.email
                                   isEqualToString: @""])
        {
        ...
        if ([friend.thumbnailURL
             hasPrefix: @"cached://"])                     // cached from address book image
        {
            ...
            userThumbnailCell.imageView.image = img;
                  ...
                  hasPrefix: @"http"])                   // includes https of course
        {
            if ([friend.thumbnailURL
                 rangeOfString: @"localhost"].location == NSNotFound)                   // is not a fake URL
            {
...
- (UITableViewCell *) tableView: (UITableView *) tableView cellForRowAtIndexPath: (NSIndexPath *) indexPath
{
    ...
    if ([friend.thumbnailURL
         hasPrefix: @"http"])                     // good for http and https
    {
        [cell.imageView
         ...
         options: SDWebImageRetryFailed];
              ...
              hasPrefix: @"cached://"])                                       // has been cached from the address book access
    {
        NSPurgeableData *pdata = [self.addressBookImageCache objectForKey: friend.thumbnailURL];
...
- (void) tableView: (UITableView *) tableView didSelectRowAtIndexPath: (NSIndexPath *) indexPath
{
    ...
    [tracker sendEventWithCategory: @"SearchFriendtoShare"
                        withAction: (lastCellPressed ? @"New" : ([friend.externalSystem
    ...
    [appDelegate.oAuthNetworkEngine emailShareWithObjectType : self.mutableShareDictionary[@"type"]
                                           ...
                                           completionHandler : ^(id no_content) {
                                               ...
                                               [appDelegate.viewStackManager
                                                presentSuccessNotificationWithMessage: notificationText];
                                               [appDelegate.viewStackManager removePopoverView];
